...
function createFileHandler(router, options) {
    ...
    return function handler(basedir, ancestors, current) {
        var abs, impl, filename, mountpath, subrouter, dir;
        ...
        if (hasRequireHandler(abs)) {
            impl = require(abs);
            if (filename !== 'index') {
                dir = path.join(basedir, ancestors, filename);
            } else {
                dir = path.join(basedir, ancestors);
            }
            ...
            if (typeof impl === 'function' && impl.length === 1) {
                ...
                subrouter.app = router.app;
                
                router.app.emit('enrouten', subrouter, Object.assign({}, options, {
                    basedir: basedir, path: dir
                }));
